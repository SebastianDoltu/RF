...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		try {
			...
			List<Plant> trainingSetPlants = null;
			
			trainingSetPlants = FileUtils.readLearningSetFromFileClass("train.txt");
			
			for(Plant p : trainingSetPlants)
			{
				System.out.println(p.toString());
			}
			
			List<Plant> evaluationSetPlants = null;
			
			evaluationSetPlants = FileUtils.readLearningSetFromFileClass("eval.txt");
			
			for(Plant p : evaluationSetPlants)
			{
				System.out.println(p.toString());
			}
			...
			// ================== calculate distances from those 3 cities to all other 13k ==============
						...
						List<Map<Plant, double[]>> distancesList = new ArrayList<Map<Plant, double[]>>();
					
						for(int i = 0; i < evaluationSetPlants.size(); i++)
						{
							Map<Plant, double[]> distances = new HashMap<Plant, double[]>();
							double[] cityDistance =  new double[trainingSetPlants.size()];
							double[] pattern = {evaluationSetPlants.get(i).GetSepalLength(), evaluationSetPlants.get(i).GetSepalWidth(),
									evaluationSetPlants.get(i).GetPetalLength(), evaluationSetPlants.get(i).GetPetalWidth()};
							for(int j = 0; j < trainingSetPlants.size(); j++)
							{
								double[] pattern2 = {trainingSetPlants.get(j).GetX(), trainingSetPlants.get(j).GetY()};
								cityDistance[j] = DistanceUtils.calculateGeneralisedEuclidianDistance(pattern, pattern2);
							}
							distances.put(evaluationSetPlants.get(i), cityDistance);
