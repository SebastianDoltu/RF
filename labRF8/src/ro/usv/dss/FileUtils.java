...
public class FileUtils 
{
	...
	private static ArrayList<String> convertLineToLearningSetRow(String line)
	{
		...
		{
			learningSetRow.add(String.valueOf(stringValues[p]));
		}
	...
	protected static List<Plant> readLearningSetFromFileClass(String fileName) throws USVInputFileCustomException
	{
		List<Plant> learningSet = null;
		try  {
			Stream<String> stream = Files.lines(Paths.get(fileName));
			learningSet = stream.map(FileUtils::convertLineToLearningSetRowClass).collect(Collectors.toList());
	...
	private static Plant convertLineToLearningSetRowClass(String line)
	{
		Plant learningSetRow;
		String[] stringValues = line.split(inputFileValuesSeparator);
	    learningSetRow = new Plant(Double.valueOf(stringValues[0]),Double.valueOf(stringValues[1]),Double.valueOf(stringValues[2]), Double.valueOf(stringValues[3]), stringValues[4] );
		return learningSetRow;
	}
